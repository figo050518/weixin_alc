...
public class RestClientErrorHandler implements ResponseErrorHandler {
    ...
    public void handleError(ClientHttpResponse response) throws IOException {
        ...
        switch (statusCode.series()) {
            case CLIENT_ERROR:{
                ...
                log.error("call client error",  ex);
                throw ex;
            }
            ...
            case SERVER_ERROR: {
                HttpServerErrorException ex = new HttpServerErrorException(statusCode, response.getStatusText(),
                        response.getHeaders(), getResponseBody(response), getCharset(response));
                log.error("call server error",  ex);
                throw ex;
            }
    ...
    public boolean hasError(ClientHttpResponse response) throws IOException {
        ...
        log.warn("hasError get error code from resp {}",errorCode);
        HttpStatus statusCode= getHttpStatusCode(response);
        return  (errorCode>0) || (statusCode.series() == HttpStatus.Series.CLIENT_ERROR ||
                statusCode.series() == HttpStatus.Series.SERVER_ERROR) ;
    }

    private int getServiceErrorCode(ClientHttpResponse response) throws IOException {
