	...
	<body>
		...
		<div class="sellerInfo">
			...
			<c:if test="${orderInfoPO.orderState=='2'||orderInfoPO.orderState=='3'}">
				...
				<div class="goodsSize" style="">
					...
					<img src="/resources/images/toNext.png" style="float:right;height:16px;margin-top:calc(6vw - 8px);margin-right:3vw;"/>
					<div class="clear"></div>
				</div>
			</c:if>
		</div> 
		...
		<div class="distributionInfoDIv" style="background: #FFFFFF;border-bottom: 1px solid #EBECEE;">
			<div class="orderMoneyDiv">
				<div class="freightDiv">
					...
					<div class="freight">¥${orderInfoPO.freightAmount}0</div>
					<div class="clear"></div>
				</div>
				...
				<c:if test="${userType=='2'}">
					<span>由用户</span>
					<span style="color: #FBBE88;">${sellerInfo.nikeName }</span>
					<span>购买</span>
				</c:if>
				<!-- 卖家 -->
				<c:if test="${userType=='1'}">
					<span>由品牌商</span>
					<span style="color: #FBBE88;">${buyerInfo.nikeName }</span>
					<span>单独配送</span>
				</c:if>
				</div>
			</div>
			<div class="buyerMessageDiv">
				<c:if test="${userType=='1'}">
					...
					<textarea placeholder="点击给商家留言" class="msgContent" autocomplete="off"></textarea>
					<div class="clear"></div>
				</c:if>
				<c:if test="${userType=='2'}">
		...
		<div class="totleCalculateDiv">
			...
			<div class="totleResult">
				<span>应收：</span><span>¥${orderInfoPO.proAmount+orderInfoPO.freightAmount}</span>
			</div>
		</div>
