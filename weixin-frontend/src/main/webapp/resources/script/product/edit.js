...
var imgboxHtml=$("#oneAddPhoto").html();
...
function release(){
	...
	var imgList=[];
	var imgs = $("#img").find(".imgbox");
	for(var i=0;i<imgs.length;i++){
		var imageId= $(imgs[i]).find("input[name='imageId']").val();
		var imageUrl = $(imgs[i]).find("input[name='imageUrl']").val();
		imgList.push({'id':imageId,'imgUrl':imageUrl});
	}
	...
	var data={'id':productId,'upState':upState,'productName':productName,'sepcList':specList,'freight':freight,'productDesc':productDesc,'group':{'id':groupId},'imageList':imgList,'fcgCateId':$("#pinLei").val(),'fcgCateName':$("#pinLeiCount").val()};
	$.ajax({
