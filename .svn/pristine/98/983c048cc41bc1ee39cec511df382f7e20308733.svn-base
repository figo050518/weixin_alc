...
public class DatabaseIntrospector {
    ...
    public List<IntrospectedTable> introspectTables(TableConfiguration tc) throws SQLException {
        ...
        while (iter.hasNext()) {
            ...
            ResultSet rs =
                    databaseMetaData.getTables(tc.getCatalog(), tc.getSchema().toUpperCase(), tc.getTableName(),
                            new String[]{"TABLE"});
            if (rs.next()) {
                introspectedTable.getFullyQualifiedTable().setRemark(rs.getString("REMARKS"));
            }
