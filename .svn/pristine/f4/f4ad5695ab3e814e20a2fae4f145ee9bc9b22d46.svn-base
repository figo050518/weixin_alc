/*
 * Diff Match and Patch Copyright 2006 Google Inc. http://code.google.com/p/google-diff-match-patch/ Licensed under the
...
public class DiffMatchPatch {
     ...
     * @param checklines Speedup flag. If false, then don't run a line-level diff first to identify the changed areas.
     *            If true, then run a faster slightly less optimal diff.
     * @param deadline Time when the diff should be complete by. Used internally for recursive calls. Users should set
     *            DiffTimeout instead.
     * @return Linked List of Diff objects.
     */
        ...
        for (int d = 0; d < pattern.length(); d++) {
            ...
            for (int j = finish; j >= start; j--) {
                ...
                if (d == 0) {
                    ...
                    rd[j] =
                            (((rd[j + 1] << 1) | 1) & charMatch) | (((last_rd[j + 1] | last_rd[j]) << 1) | 1)
                                    | last_rd[j + 1];
                }
                if ((rd[j] & matchmask) != 0) {
    ...
    public Object[] patch_apply(LinkedList<Patch> patches, String text) {
        ...
        for (Patch aPatch : patches) {
            ...
            if (text1.length() > Match_MaxBits) {
                ...
                if (start_loc != -1) {
                    end_loc =
                            match_main(text, text1.substring(text1.length() - Match_MaxBits),
     ...
     * URLEncoder.encode which returns uppercase hex. Example: "%3F" -> "?", "%24" -> "$", etc.
     * 
