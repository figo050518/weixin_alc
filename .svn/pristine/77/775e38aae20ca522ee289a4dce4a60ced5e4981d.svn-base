...
function pullupRefresh() {
	setTimeout(function() {
		...
		for(var e in response){
			...
			if(response[e].genEvent=="1"){
				htm+="<div class='price'><span>-¥"+response[e].genAmount+"</span></div>";
			}
			if(response[e].genEvent=="0"){
				htm+="<div class='price'><span>+¥"+response[e].genAmount+"</span></div>";
			}
			if(response[e].bussOrderType=="1"){
				htm+="<div class='status'><span>提现</span></div>";
			}
			if(response[e].bussOrderType=="2"){
				htm+="<div class='status'><span>平台订单</span></div>";
			}
			if(response[e].bussOrderType=="3"){
				htm+="<div class='status'><span>自营订单</span></div>";
			}
	...
	if(response==0 && obj=="0"){
		...
		for(var e in response){
			html+="<li class='mui-table-view-cell' style='padding: 0px;background: #FFFFFF;width:100vw;position: relative;' id='"+response[e].id+"'>"+
						...
						"<div class='time'><span>"+response[e].addTime+"</span></div>";
						if(response[e].genEvent=="1"){
							html+="<div class='price'><span>-¥"+response[e].genAmount+"</span></div>";
						}
						if(response[e].genEvent=="0"){
							html+="<div class='price'><span>+¥"+response[e].genAmount+"</span></div>";
						}
						if(response[e].bussOrderType=="1"){
							html+="<div class='status'><span>提现</span></div>";
						}
						if(response[e].bussOrderType=="2"){
							html+="<div class='status'><span>平台订单</span></div>";
						}
						if(response[e].bussOrderType=="3"){
							html+="<div class='status'><span>自营订单</span></div>";
						}
...
function getResponseData(obj){
	...
	$.ajax({
		type:'get',
		url:'/uc/finance/transaction/list',
		data:{"pageIndex":pageIndex},
		async:false,
		success:function(obj){
			//console.log(obj);
			response = obj.transactionList;
		}
	});
	return response;	
}
