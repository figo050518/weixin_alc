 ...
 * the execution environment (i.e. an Eclipse plugin, and Ant task, a NetBeans plugin, etc.) The default ShellCallback
 * that is very low function and does not support the merging of Java files. The default shell callback is appropriate
 * for use in well controlled environments where no changes made to generated Java files.
 * 
 * @author Jeff Butler
 */
public interface ShellCallback {
     ...
     * class to cache results. The returned <code>java.io.File</code> object:
     * <ul>
     ...
     *             case, the generator will not save the file it is currently working on. The generator will add the
     ...
     * the merged source (formatted). The generator will write the merged source as-is to the file system. A merge
     ...
     * @throws ShellException if the file cannot be merged for some reason. If this exception is thrown, nothing will be
     *             saved and the existing file will remain undisturbed. The generator will add the exception message to
     ...
     * have been created or updated. If you are running outside of an IDE, your implementation need not do anything in
     * this method.
     * 
